...
def chats_group() -> None:
    ...
    \b
    Common examples:
      prowlr chats list                    # List all chats
      prowlr chats list --user-id alice    # Filter by user
      prowlr chats get <chat_id>           # View details
      prowlr chats create --session-id s1 --user-id u1
      prowlr chats delete <chat_id>        # Delete a chat
    """
...
def list_chats(
    ...
    Examples:
      prowlr chats list
      prowlr chats list --user-id alice
      prowlr chats list --channel discord
      prowlr chats list --user-id alice --channel discord
    """
...
def get_chat(
    ...
    \b
    CHAT_ID  Chat UUID, obtainable via `prowlr chats list`.
...
def create_chat(
    ...
    Inline creation examples:
      prowlr chats create --session-id "discord:alice" \\
...
def update_chat(
    ...
    \b
    Examples:
      prowlr chats update <chat_id> --name "Renamed Chat"
    """
    base_url = _base_url(ctx, base_url)
...
def delete_chat(
    ...
    \b
    Examples:
      prowlr chats delete 823845fe-dd13-43c2-ab8b-d05870602fd8
    """
    base_url = _base_url(ctx, base_url)
    with client(base_url) as c:
