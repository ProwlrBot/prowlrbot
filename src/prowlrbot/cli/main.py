...
_t = time.perf_counter()
from .acp_cmd import acp_cmd  # noqa: E402

_record(".acp_cmd", time.perf_counter() - _t)

_t = time.perf_counter()
from .market_cmd import market_group  # noqa: E402

_record(".market_cmd", time.perf_counter() - _t)

_t = time.perf_counter()
from .agent_cmd import agent_group  # noqa: E402

_record(".agent_cmd", time.perf_counter() - _t)

_t = time.perf_counter()
from .team_cmd import team_group  # noqa: E402

_record(".team_cmd", time.perf_counter() - _t)

_t = time.perf_counter()
from .studio_cmd import studio_cmd  # noqa: E402
...
def cli(ctx: click.Context, host: str | None, port: int | None) -> None:
    """ProwlrBot CLI — Always watching. Always ready."""
