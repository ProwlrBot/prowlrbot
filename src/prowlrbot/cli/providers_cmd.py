...
def configure_provider_api_key_interactive(
    ...
    if defn.is_local:
        ...
        click.echo(
            "Use 'prowlr models download' to add models, "
            "then 'prowlr models set-llm' to activate.",
        )
        return provider_id
...
def configure_llm_slot_interactive(*, use_defaults: bool = False) -> None:
    ...
    if not eligible:
        if use_defaults:
            click.echo(
                "No LLM provider configured. Run 'prowlr models config' "
                "to configure later.",
            )
            return
        click.echo(
    ...
    if not model and use_defaults:
        click.echo(
            ...
            "Run 'prowlr models config' to set one.",
        )
        return
    ...
    for defn in list_providers():
        ...
        if defn.is_local:
            ...
            if all_models:
                ...
                click.echo("  Use 'prowlr models download' to add models.")
        else:
            if defn.is_custom or not defn.default_base_url:
...
def add_provider_cmd(
    ...
    click.echo(
        "  Run 'prowlr models add-model' to add models, "
        "then 'prowlr models config-key' to set the API key.",
    )
...
def download_cmd(
    ...
    try:
        ...
        click.echo(
            click.style(
                ...
                "Install with: pip install 'prowlrbot[local]'",
                fg="red",
            ),
        )
        raise SystemExit(1) from exc
    ...
    click.echo(
        ...
        f"  prowlr models set-llm  (select '{backend}' provider)",
    )
...
def list_local_cmd(backend: str | None) -> None:
    ...
    if not models:
        ...
        click.echo("Use 'prowlr models download <repo_id>' to download one.")
        return
...
def ollama_list_cmd() -> None:
    ...
    if not models:
        ...
        click.echo("Use 'prowlr models ollama-pull <model>' to download one.")
        return
...
def ollama_remove_cmd(model_name: str, yes: bool) -> None:
    ...
    Examples:
      ...
      prowlr models ollama-remove qwen2.5:3b -y
    """
    from ..providers.ollama_manager import OllamaModelManager
