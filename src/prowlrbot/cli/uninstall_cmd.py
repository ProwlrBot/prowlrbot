...
def _remove_path_entry(profile: Path) -> bool:
    """
    Remove ProwlrBot PATH lines from a shell profile. Returns True if changed.
    """
    if not profile.is_file():
        return False

    text = profile.read_text()
    # Remove the "# ProwlrBot" comment line and the export PATH line
    cleaned = re.sub(
        r"\n?# ProwlrBot\nexport PATH=\"\$HOME/\.prowlrbot/bin:\$PATH\"\n?",
        "\n",
        text,
    )
    if cleaned == text:
...
def uninstall_cmd(purge: bool, yes: bool) -> None:
    """Remove ProwlrBot environment, CLI wrapper, and shell PATH entries."""
    wd = WORKING_DIR

    if purge:
        click.echo(f"This will remove ALL ProwlrBot data in {wd}")
    else:
        click.echo(
            "This will remove the ProwlrBot Python environment and CLI wrapper.",
        )
    ...
    click.echo("")
    click.echo("ProwlrBot uninstalled. Please restart your terminal.")
