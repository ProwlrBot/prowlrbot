        ...
        def try_lock(agent_data):
            try:
                result = engine.lock_file(
                    "src/contested.py", agent_data["agent_id"], room["room_id"]
                )
                with lock:
                    results.append(result)
            except Exception:
                with lock:
                    results.append(LockResult(success=False, reason="exception"))
    ...
    def test_concurrent_claim_task(self, engine, room):
        ...
        def try_claim(agent_data):
            try:
                ...
                with lock:
                    results.append(ClaimResult(success=False, reason="exception"))
...
class TestEventRetention:
    ...
    def test_purge_keeps_recent(self, engine, room, agent):
        ...
        engine.broadcast_status(room["room_id"], agent["agent_id"], "recent message")
...
class TestEventCallback:
    ...
    def test_callback_exception_doesnt_break_engine(self, engine, room, agent):
        ...
        engine.broadcast_status(room["room_id"], agent["agent_id"], "still works")
        events = engine.get_events(room["room_id"])
    ...
    def test_no_callback_is_fine(self, engine, room, agent):
        ...
        engine.broadcast_status(room["room_id"], agent["agent_id"], "no callback")
        events = engine.get_events(room["room_id"])
        assert len(events) >= 1
...
class TestEdgeCases:
    def test_register_agent_special_characters(self, engine, room):
        ...
        agent = engine.register_agent("agent'; DROP TABLE agents;--", room["room_id"])
    ...
    def test_task_title_special_characters(self, engine, room):
        task = engine.create_task(room["room_id"], "Test <script>alert('xss')</script>")
    ...
    def test_context_value_large_but_bounded(self, engine, room, agent):
        ...
        big_value = "x" * 50000
        engine.set_context(room["room_id"], agent["agent_id"], "big-key", big_value)
